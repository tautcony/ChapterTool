...
### Requirement: Predecessor settings files are ignored
When `settings.json` is absent, the system SHALL NOT import predecessor settings files into the current unified document.

#### Scenario: Predecessor files do not create unified settings
- **WHEN** `appsettings.json`, `theme-settings.json`, or `font-settings.json` exists and `settings.json` is absent
- **THEN** typed loads SHALL return current defaults
- **AND** loading alone SHALL NOT create a configuration file
- **AND** the predecessor files SHALL remain unchanged
