...
- 引入新的章节表达式引擎和 Lua 实现，支持时间、帧率、章节索引等上下文，并限制脚本执行预算。

### 问题改进

- 将章节时间属性统一为 `StartTime` 和 `EndTime`，并抽象媒体章节读取边界，改进媒体容器导入的可替换性。
- 加强设置文件、外部导入器、核心 API 和 CLI 启动边界的校验与失败诊断，保留损坏设置并避免不安全的进程参数处理。

### 其他

- 准备发布 `ChapterTool.Core` NuGet 包，支持 .NET 8、.NET 9 和 .NET 10，包含包内 README、许可证、文档和符号包配置。
- 重构章节导入、编辑和导出模型，引入结构化导入进度、诊断代码、诊断来源及统一的导入导出格式描述。
- 接入 Sentry 启动诊断，使用源生成 JSON 序列化优化运行时性能，并整理 Avalonia Headless 测试结构。
- 优化跨平台 CI、发布脚本和应用发布配置，移除已废弃的旧版安装包资产。
