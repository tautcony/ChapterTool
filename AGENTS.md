...
- Treat `src/ChapterTool.Avalonia.UI/Localization/Resources/Locales/*.axaml` as the source for shared UI translations. After changing an AXAML locale, run `python3 scripts/axaml-to-json.py` to regenerate the matching Wasm JSON files under `src/ChapterTool.Wasm/Resources/Locales/`. Run `python3 scripts/axaml-to-json.py --check` to verify that generated JSON files are synchronized. Do not edit generated Wasm JSON files by hand. CLI JSON resources contain CLI-specific entries and are not generated by this script.
