...
def load_sarif(path):
    ...
    decoder = json.JSONDecoder(); text = path.read_text(encoding="utf-8"); runs = []; index = 0
    while True:
        while index < len(text) and text[index].isspace(): index += 1
        if index >= len(text): break
        obj, end = decoder.raw_decode(text, index); index = end; runs.extend(obj.get("runs", []))
...
def result_location(item):
    ...
    loc = (item.get("locations") or [{}])[0]; region = {}
...
def uri_to_path(uri):
    ...
    if parsed.scheme and parsed.scheme != "file": return uri
...
class _SourceMasker:
    ...
    def __call__(self, line):
        if self.in_block_comment:
            ...
            if end is None: return " " * len(line)
            self.in_block_comment = False
...
def enclosing_type(source_path, target_line):
    ...
    if source_path not in _type_cache:
        try: lines = Path(source_path).read_text(encoding="utf-8").splitlines()
        except OSError: return ""
        masker = _SourceMasker()
        depth_after = [0] * (len(lines) + 1); types = []; depth = 0
        for i, line in enumerate(lines, 1):
            masked = masker(line)
            match = _TYPE_RE.search(masked)
            if match: types.append((match.group(1), i, depth, "{" in masked[match.end():]))
            depth += masked.count("{") - masked.count("}"); depth_after[i] = depth
        _type_cache[source_path] = (depth_after, types)
    depth_after, types = _type_cache[source_path]
    if target_line < 1 or target_line > len(depth_after) - 1: return ""
    best, best_depth = "", -1
    for name, decl, d, body_on_decl in types:
        if target_line < decl: continue
        if body_on_decl:
            ...
            if open_at is None: continue
            close = next((k for k in range(open_at + 1, len(depth_after)) if depth_after[k] == d), len(depth_after) - 1)
        if target_line > close: continue
        if d > best_depth: best, best_depth = name, d
    return best
...
def main():
    ...
    sources = [root / p for p in ("src/ChapterTool.Contracts/ChapterTool.Contracts.csproj", "src/ChapterTool.Core/ChapterTool.Core.csproj", "src/ChapterTool.Infrastructure/ChapterTool.Infrastructure.csproj", "src/ChapterTool.CommandLine/ChapterTool.CommandLine.csproj", "src/ChapterTool.Avalonia.UI/ChapterTool.Avalonia.UI.csproj", "src/ChapterTool.Avalonia/ChapterTool.Avalonia.csproj")]
    tests = [root / p for p in ("tests/ChapterTool.TestSupport/ChapterTool.TestSupport.csproj", "tests/ChapterTool.Core.Tests/ChapterTool.Core.Tests.csproj", "tests/ChapterTool.Infrastructure.Tests/ChapterTool.Infrastructure.Tests.csproj", "tests/ChapterTool.CommandLine.Tests/ChapterTool.CommandLine.Tests.csproj", "tests/ChapterTool.Avalonia.Tests/ChapterTool.Avalonia.Tests.csproj", "tests/ChapterTool.Avalonia.Headless.Tests/ChapterTool.Avalonia.Headless.Tests.csproj", "tests/ChapterTool.Wasm.Tests/ChapterTool.Wasm.Tests.csproj")]
    projects = [(p, p in tests) for p in sources + tests]
    for project, _ in projects:
        if not project.is_file(): raise FileNotFoundError(f"project was not found at {project}")
    shutil.rmtree(raw_dir, ignore_errors=True); raw_dir.mkdir(parents=True)
    last_code = 0
    for project, is_test in projects:
        raw = raw_dir / project.stem  # per target framework: <stem>.<tfm>.sarif
        for old in raw_dir.glob(f"{project.stem}.*.sarif"): old.unlink()
        command = ["dotnet", "build", str(project), "--configuration", args.Configuration, f"-p:ReportErrorLog={raw}"]
        if is_test: command.append("--no-dependencies")
        else: command.append("-p:BuildProjectReferences=false")
        if args.Rebuild: command.append("-t:Rebuild")
    ...
    runs = []
    for raw in sorted(raw_dir.glob("*.sarif")): runs.extend(load_sarif(raw).get("runs", []))
    ...
    for run in runs:
        for item in run.get("results", []):
            ...
            if uri:
                ...
                if path.startswith(str(root)): path = os.path.relpath(path, root)
                file = path
            else:
                file = "<no-file>"
            type_name = enclosing_type(uri_to_path(uri), line) if uri else ""
            key = (rule, file, line, column, str(message))
            if key in seen: continue
            seen.add(key)
            diagnostics.append({"ruleId": rule, "level": item.get("level") or "none", "message": str(message), "file": file, "line": line, "column": column, "type": type_name})
    ...
    if not diagnostics: print("No matching diagnostics were found."); return last_code
    for title, key in (("By rule", "ruleId"), ("By severity", "level"), ("By file", "file"), ("By type", "type")):
        print(f"\n{title}")
        for value, count in sorted(Counter(item[key] for item in diagnostics).items()): print(f"  {value or '<unknown>'} {count}")
    print("\nDiagnostics")
