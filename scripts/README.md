...
| `publish.sh` | Bash; Unix-like hosts or Git Bash | .NET SDK | Publish Linux, macOS, or Windows runtime artifacts. macOS bundles require a macOS host. |
| `publish.ps1` | PowerShell 7; Windows | .NET SDK | Publish Windows runtime artifacts. |
| `generate-app-icons-macos.sh` | Bash; macOS only | ImageMagick, `iconutil` | Generate ICNS and ICO files from the SVG icon source. |
| `verify-publish-artifact.sh` | Bash; Unix-like hosts | Bash | Manually verify a published artifact layout. |
