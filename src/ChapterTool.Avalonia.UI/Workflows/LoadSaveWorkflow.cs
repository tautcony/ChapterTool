...
internal sealed class LoadSaveWorkflow(
    ...
    IChapterSourceLoader? sourceLoader = null)
{
    public async ValueTask<LoadWorkflowResult> LoadAsync(
        ...
        => await LoadAsync(
            new LocalPathChapterSource(path),
            reportProgress,
            cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await LoadSourceAsync(
            source,
    ...
    public async ValueTask<AppendWorkflowResult> AppendAsync(string path, CancellationToken cancellationToken)
        => await AppendAsync(new LocalPathChapterSource(path), cancellationToken);

    public async ValueTask<AppendWorkflowResult> AppendAsync(ChapterSourceDocument source, CancellationToken cancellationToken)
    {
    ...
    private ValueTask<ChapterImportResult> LoadSourceAsync(
        ChapterSourceDocument source,
        IChapterImportProgressReporter? progress,
        CancellationToken cancellationToken)
    {
        if (sourceLoader is not null)
        {
            return sourceLoader.LoadAsync(source, progress, cancellationToken);
        }

        return source is LocalPathChapterSource local
            ? loadService.LoadAsync(local.NormalizedPath, progress, cancellationToken)
            : ValueTask.FromResult(ChapterImportResult.Failed(new ChapterDiagnostic(
                DiagnosticSeverity.Error,
                ChapterDiagnosticCode.UnsupportedInput,
                "This host cannot load buffered chapter sources.")));
    }
