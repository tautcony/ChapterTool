...
import { ChapterTool } from "../src/index.ts";
import type { ChapterSet } from "../src/types.ts";
...
describe("ChapterTool Core API mapping", () => {
  ...
  it("exposes Core frame and expression operations", async () => {
    ...
    // Without caretIndex the default falls back to expression.length.
    expect((await tool.analyzeExpression("t + ")).spans.length).toBeGreaterThan(0);
