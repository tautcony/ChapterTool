...
| Language selection | Provides no UI localization | Uses shared `.axaml` resources | Uses shared `.axaml` resources | Uses Web-only `.json` resources | `ChapterTool.Localization`, `AvaloniaLocalizationResourceAdapter`, and `WasmLocalizer` `[Host variant]` |
